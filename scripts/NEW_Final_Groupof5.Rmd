...
crp_by_sex %>% 
  ...
  labs(x = "Diabetes Treatment",
       y = "CRP Level (Female)",
       title = "Diabetes Treatment and Female CRP by HbA1c")+
  theme(plot.title = element_text(hjust = 0.5))+
  scale_x_discrete(labels = c("Diet & Exerc.", "Medication", "No Treatment"))+
...
```{r RQ3-brittany, fig.cap = "(ref:RQ3-brittany)"}
...
(ref:RQ3-brittany) Here goes the caption.
...
#Null: There is no relation between visiting the doctor in the last year and inflammation levels among those that partake in diet and exercise as a treatment for diabetes. 
RQ4df <- data %>% 
...
prop_access_bar <- 
  ggplot(data = RQ4df, aes(x = access, fill = crp2))+
  geom_bar(position = "dodge")+
  labs(title = "Relation of Inflammation Levels low/high and Seeing a Doctor in the Last Year Y/N", subtitle= "Among Individuals with Diabetes that use Diet and Exercise as Treatment", x = "Saw a doctor in the last year Y/N", y = "Frequency")
prop_access_bar+
...
here [@here], rio[@chanRioSwissarmyKnife2021a], tidyverse[re], ggpubr [@kassambaraGgpubrGgplot2Based2022],bibtex [@francoisBibtexBibtexParser2022],papaja [@austPapajaPrepareReproducible2022], psych
[@revellePsychProceduresPsychological2022],and forcats [@wickhamForcatsToolsWorking2022], janitor[@firkeJanitorSimpleTools2021], gt [@iannoneGtEasilyCreate2022], stargazer[@hlavacStargazerWellformattedRegression2018]. 
