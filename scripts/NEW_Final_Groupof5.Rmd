...
filter(crp <5, age > 50) %>% 
 ggplot(aes(hba1c, crp)) + geom_point(aes(color = diabetes_treat), size = 3) +
  theme_minimal() +
  scale_color_brewer(palette = 15, name = "Diabetes Treatment", labels = c("Diet & Exercise", "Medication", "No Treatment")) + theme_apa()
plot.a + font("xlab", size = 14) + font("ylab", size = 14)+ font("xy.text", size = 14, color = "black", face = "bold") +
  labs( title = "Relationship Between CRP and hba1c by Diabetes Treatment Type", 
