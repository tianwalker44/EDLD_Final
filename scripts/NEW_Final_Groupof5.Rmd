...
data <- data <- data %>% select(sex, diagnosis, age, hba1c, crp, medication, dt_exrcse, q5026, q5027) %>% rename(care = q5026, num_care = q5027) %>% filter(age >=50 & (diagnosis == 1 & crp < 5))  %>% as_tibble #Selecting only relevant variables, renaming for clarification, and filtering to include diagnosed people who have high levels of inflammation (crp). 
...
```{r RQ1}
#RQ1: Does diet and exercise predict inflammation levels? (Requires filtering, modeling, a table, and a figure. Do only as much as possible in this timeframe!)
...
```{r RQ2}
#RQ2: Does taking medication lower inflammation levels? (Requires filtering, modeling, a table, and a figure. Do only as much as possible in this timeframe!)

RQ2_df <- data %>%
  filter(!is.na(medication) & !is.na(hba1c) & !is.na(age), !(is.na(crp)))

mod2 <- lm(crp~hba1c*medication, data = RQ2_df)
