...
data <- data %>% select(sex, diagnosis, age, hba1c, crp, medication, dt_exrcse) %>% filter(age >=50 & (diagnosis == 1 | hba1c >6.5))  %>% as_tibble #Selecting only relevant variables, renaming for clarification, and filtering. Once we figure out where the access variable (Q5004) is, we can replace this with the code below.
...
mod1 <- lm(crp~hba1c*dt_exrcse, data = RQ2_df)
