...
data <- import(here("data","mexico_raw_dat.csv")) %>% as_tibble #Importing the Mexico dataset
RQ3df <- data %>% 
  filter(!is.na(dt_exrcse) & !is.na(hba1c) & !is.na(age), !(is.na(crp)), !is.na(q5026))
str(RQ3df)  
RQ3df <- RQ3df %>% select(diagnosis, age, hba1c, crp, dt_exrcse, q5026) %>% rename(care = q5026) %>% filter(age >=50 & (diagnosis == 2 | crp < 5))
str(RQ3df)
RQ3df$care <- factor(RQ3df$care,
                      levels = c(1,2), labels = c("Yes","No")) #How do i get rid of NA values
table(RQ3df$care)

prop <- prop.table(table(RQ3df$care))
